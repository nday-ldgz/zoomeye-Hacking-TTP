...
![image](https://img.shields.io/github/followers/nday-ldgz?style=social)![image](https://img.shields.io/github/forks/nday-ldgz/zoomeye-Hacking-TTP?style=social)![image](https://img.shields.io/github/watchers/nday-ldgz/zoomeye-Hacking-TTP?style=social)
...
ZoomEye对于注册用户开放的数据权限比同类产品更多，相当于送了一个普通会员，注册会员web查询结果展示400条，api数据额度1w/月，基本满足对数据量需求不大的用户进行攻防上的数据挖掘，对挖掘的数据信息进行消化判断利用。

对于需求量大的用户和需要特定权限和功能的用户可以开通高级会员、VIP会员以下是价格对比、数据消费功能对比、和基础功能对比表。

价格对比：
| ID| 注册用户 | 高级用户 | VIP用户| 
| --- | --- | --- |--- |
| 1 | - | 500RMB/月 |1000RMB/月 |
...
| ID| 注册用户 | 高级用户 | VIP用户| 
| --- | --- | --- |--- |
| 1 | 查询结果展示量400条 | 查询结果展示量1000条 |查询结果展示量2000条 |
| 2 | API数据总额度免费10000条/月 |  API数据总额度免费10000条/月 +2W条数据 | API数据总额度免费10000条/月+3w条数据|
| 3 | API查询结果数据比率100% | API查询结果数据比率100%  | API查询结果数据比率100%  |
| 4 | 数据订阅规格2个订阅/20个订阅IP |  数据订阅规格5个订阅/50个订阅IP | 数据订阅规格10个订阅/256个订阅IP |
...
| ID| 注册用户 | 高级用户 | VIP用户| 
| --- | --- | --- |--- |
| 1 | - | 300RMB/永久 |1000RMB/永久 |

数据消费功能对比：

| ID| 注册用户 | 普通用户 | 高级用户| 
| --- | --- | --- |--- |
| 1 | 网站查询数据量50条 | 网站查询数据量10,000条 |网站查询数据量10,000条 |
| 2 | API查询数据1F币（最多10,000条）/次 |  API查询数据免费前100条/次 | API查询数据免费前10000条/次|
| 3 |数据存储时长-- |数据存储时长7天 | 数据存储时长14天  |

基础功能对比表：
| ID| 注册用户 | 普通用户 | 高级用户| 
| --- | --- | --- |--- |
| 1 | 证书有效性提取验证 | 证书有效性提取验证 |一键排除干扰数据 |
| 2 | 自定义语法查询 | 自定义语法查询 | 证书有效性提取验证  |
| 3 | 邮件技术支持 | 通过客户端执行全球扫描(需认证) | 自定义语法查询 |
| 4 | -  |  生成全球范围受影响报告 | 正则表达式语法查询 |
| 5 | -  |  提交PoC拿奖金 |精准匹配icon |
| 6 | -  | 邮件技术支持 |port_size语法查询  |
| 7 | -  | 电话技术支持 |通过客户端执行全球扫描(需认证) |
| 8 | -  | - |生成全球范围受影响报告 |
| 9 | -  | - |提交PoC拿奖金 |
| 10 | -  | - |邮件技术支持 |
| 11 | -  | - |电话技术支持 |
