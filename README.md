...
ZoomEye对于注册用户开放的数据权限比同类产品更多，web查询结果展示400条，api数据额度1w/月，基本满足对数据量需求不大的用户进行攻防上的数据挖掘，对挖掘的数据信息进行消化判断利用。
...
| ID| 注册用户 | 高级用户 | VIP用户| 
| --- | --- | --- |--- |
| 1 | 查询结果展示量400条 | 查询结果展示量400条 |查询结果展示量400条 |
...
| 3 | - | 通过客户端执行全球扫描(需认证) | 自定义语法查询 |
| 4 | -  |  生成全球范围受影响报告 | 正则表达式语法查询 |
| 5 | -  |  提交PoC拿奖金 |精准匹配icon |
| 6 | -  | 邮件技术支持 |port_size语法查询  |
| 7 | -  | 电话技术支持 |通过客户端执行全球扫描(需认证) |
| 8 | -  | 电话技术支持 |通过客户端执行全球扫描(需认证) |
| 9 | -  | 电话技术支持 |通过客户端执行全球扫描(需认证) |
| 10 | -  | 电话技术支持 |通过客户端执行全球扫描(需认证) |
